...
export const Sidebar: React.FC = () => {
  ...
  const navItems: { id: NavTab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'tools', label: 'AI Tools', icon: <Wand2 className="w-4 h-4" /> },
    ...
    <aside className="w-64 bg-slate-950 border-r border-slate-800/80 flex flex-col justify-between h-screen p-4 shrink-0 select-none">
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between px-2 pt-2">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-slate-100 text-lg tracking-tight">AI Video Magic</span>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-4 pt-4 border-t border-slate-800/60">
        ...
        <div className="p-3.5 rounded-xl bg-gradient-to-b from-indigo-950/40 to-slate-900/80 border border-indigo-900/40 relative overflow-hidden">
          ...
          <h4 className="text-sm font-semibold text-slate-100 mb-1">Upgrade to Pro</h4>
          <p className="text-xs text-slate-400 mb-3 leading-relaxed">
            Unlock all features, 4K rendering & remove watermark.
          </p>
          <button className="w-full py-2 px-3 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-semibold shadow-md shadow-purple-900/30 transition-all flex items-center justify-center gap-1.5">
